...
//   Legend as PieLegend,
//   ResponsiveContainer,
//   AreaChart, // Thay LineChart bằng AreaChart cho đẹp hơn
//   Area,
//   XAxis,
//   YAxis,
//   CartesianGrid,
...
//     phongNoTien: 0,
//   });
//   const [chartData, setChartData] = useState([]); // State cho biểu đồ tiêu thụ
...
//           axios.get("http://localhost:5000/api/thong-ke", config),
...
//   // Dữ liệu biểu đồ tròn lấy từ stats thật
//   const pieData = [
//     { name: "Đang ở", value: stats.tongSinhVien },
//     { name: "Còn trống", value: stats.choConTrong },
//   ];
//   const PIE_COLORS = ["#2b78c5", "#27ae60"]; // Xanh dương (Đang ở) và Xanh lá (Trống)
...
//                 <YAxis
//                   yAxisId="right"
//                   orientation="right" // Đưa sang bên phải
//                   axisLine={false}
//                   tickLine={false}
...
export default function Dashboard() {
  // 1. Thêm các trường dữ liệu phòng vào state mặc định để tránh lỗi undefined
  const [stats, setStats] = useState({
    tongSinhVien: 0,
    thietBiHong: 0,
    choConTrong: 0,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 2. SỬA LỖI 404: Đổi đường dẫn URL cho chuẩn xác với Backend
        const [resStats, resChart] = await Promise.all([
  ...
  // 3. SỬA LỖI LOGIC BIỂU ĐỒ TRÒN: Dùng đồng nhất số lượng PHÒNG
  const phongDangO =
  ...
  // Màu sắc: Đỏ (Đầy), Xanh lá (Trống), Xanh dương (Đang ở)
